  ...
  while(!quit)
  {
    ...
    switch((char)insn.itype.opcode) // the cast is just to suppress warnings about the unsupported opcodes
    {
      ...
      case OPCODE_LW:
        ...
        break;
      case OPCODE_SW:
        if (verbose && dmem[reg[insn.itype.rs] + insn.itype.imm] != reg[insn.itype.rt]) {
          // Modification by Jonathan Piland, 1 March 2024
          cout << "Write RAM[0x" 
          << hex << setw((WORD_BITS+3)/4) << setfill('0') << reg[insn.itype.rs] + insn.itype.imm << dec
          << "] = 0x" 
          << hex << setw((WORD_BITS+3)/4) << setfill('0') << reg[insn.itype.rt] << dec << "\n";
        }
      ...
      case OPCODE_OUTPUT:
        ...
        // // changed sp24 - Jonathan Piland
        // // optional modification below:
        // cout << "Output char #" << reg[insn.rtype.rs] << "\n";
        break;
      case OPCODE_INPUT:
